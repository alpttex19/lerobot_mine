...
class EnvConfig(draccus.ChoiceRegistry, abc.ABC):
    ...
    def create_envs(
        ...
        if self.gym_id not in gym_registry:
            print(
                f"gym id '{self.gym_id}' not found, attempting to import '{self.package_name}'..."
            )
            try:
...
class LiberoEnv(EnvConfig):
    ...
    def __post_init__(self):
        ...
        if self.camera_name_mapping is not None:
            ...
            mapped_eye_in_hand = self.camera_name_mapping.get(
                "robot0_eye_in_hand_image", "image2"
            )
            self.features_map[LIBERO_KEY_PIXELS_AGENTVIEW] = (
                f"{OBS_IMAGES}.{mapped_agentview}"
            )
            self.features_map[LIBERO_KEY_PIXELS_EYE_IN_HAND] = (
                f"{OBS_IMAGES}.{mapped_eye_in_hand}"
            )
...
class IsaaclabArenaEnv(HubEnvConfig):
    ...
    def get_env_processors(self):
        state_keys = tuple(
            k.strip() for k in (self.state_keys or "").split(",") if k.strip()
        )
        camera_keys = tuple(
            k.strip() for k in (self.camera_keys or "").split(",") if k.strip()
        )
        if not state_keys and not camera_keys:
            raise ValueError(
                "At least one of state_keys or camera_keys must be specified."
            )
        return (
            PolicyProcessorPipeline(
                steps=[
                    IsaaclabArenaProcessorStep(
                        state_keys=state_keys, camera_keys=camera_keys
                    )
                ]
            ),
            PolicyProcessorPipeline(steps=[]),
